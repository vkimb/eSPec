...
eSPecPATH=/home/vinicius/eSPec_v0.6/espec.x
WDIR=$DIR
OPTC=0
OPTT=0
OPTO=0
...
usage (){
	  ...
	  echo ' -cw       Compress only the wavepacket ReIm_****.dat and the standart input and output files'
#	  echo ' -c        Compress the output data'
	  echo ' -t        Run the calculations at tmp (working dir is the default)'
	  echo ' -s        Run the calculations at /scr (working dir is the default)'
...
if [ $OPTT -eq 1 ]; then
   ...
   n1=`cat -n $INPUT | grep .FILE | awk '{printf $1}'`
   n2=$(echo " $n1 +1" | bc)
   FILEPOT=`cat -n $INPUT | grep $n2 | awk '{printf $2}'`
   n1=`cat -n $INPUT | grep .READ | awk '{printf $1}'`
   n2=$(echo " $n1 +1" | bc)
   FILEDIP=`cat -n $INPUT | grep $n2 | awk '{printf $2}'`
   cp $FILEPOT $TDIR 
   cp $FILEDIP $TDIR
fi
...
#if [ $OPTT -eq 1 ]; then
if [ $OPTT -ne 0 ]; then
   cd $TDIR; time nice -$NICE $eSPecPATH > $OUTPUT
   if [ $OPTC -eq 0 ]; then 
   ...
   time nice -$NICE $eSPecPATH > $OUTPUT
fi
##
echo
...
#   DATEF=`date '+%d%m%Y-%l:%M'`
    ...
    echo "Compressing the output data to \"${COUT}\""
    if [ $OPTT -eq 1 ]; then
	cd $TDIR; tar cf - ReIm* movie.gplt pulse.dat $FILEI $OUTPUT $INPUT \
	    | gzip > ${COUT}
	if [ $OPTK -eq 0 ]; then if [ -f $COUT -o $OUTPUT ]; then rm -rf $TDIR; fi; fi
    else
	tar cf - ReIm*  movie.gplt pulse.dat $FILEI $OUTPUT $INPUT \
	    | gzip > ${COUT}
	if [ -f $COUT ]; then rm -f ReIm*dat veff*dat eigvc*dat movie.gplt pulse.dat $OUTPUT; fi 
#	 if [ -f ReIm_0001.dat  ]; then rm -f ReIm*; fi
#	 if [ -f veff_0001.dat  ]; then rm -f veff*; fi
